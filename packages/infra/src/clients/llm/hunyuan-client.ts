...
export class HunyuanClient implements LLMClient {
  ...
  private baseUrl = process.env.HUNYUAN_BASE_URL || 'https://api.hunyuan.cloud.tencent.com/v1';

  constructor(apiKey: string, model = 'hunyuan-large') {
    this.apiKey = apiKey;
    this.model = model;
  }
  ...
  async analyze(prompt: { system: string; context: string; outputSchema: string }): Promise<LLMAnalysisResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Hunyuan API error: ${response.status} - ${err}`);
    }
    ...
    return engine.parseResponse('hunyuan', this.model, content, latency, {
